...
@pytest.mark.asyncio
async def test_sell_style_clamps_to_band_low(tmp_path, runtime_cache, app_config):
    runtime_cache.add_contract("NIFTY", "2024-07-11", 22650, "CE", lot=50, tick=0.05, low=0.05, high=500.0)
    store = SQLiteStore(tmp_path / "state.sqlite", run_id="sell-style")
    broker = DummyBroker()
    metric = NullMetric()
    metrics = SimpleNamespace(
        order_queue_depth=metric,
    ...
    submit_cfg = OMSSubmitConfig(default="auto", max_spread_ticks=2, depth_threshold=50)
    cfg = OMSConfig(resubmit_backoff=0.1, reconciliation_interval=1.0, max_inflight_orders=10, submit=submit_cfg)
    default_meta = (0.05, 50, 0.05, 500.0)
    engine = OMS(broker=broker, store=store, config=cfg, metrics=metrics, default_meta=default_meta, square_off_time=None)
    symbol = "NIFTY-2024-07-11-22650CE"
    engine.update_market_depth(symbol, bid=0.05, ask=0.10, bid_qty=10, ask_qty=10)
    order = await engine.submit(
        strategy="test",
        symbol=symbol,
        side="SELL",
    ...
    assert order.limit_price == pytest.approx(0.05)
    assert broker.submissions[0].limit_price == pytest.approx(0.05)
