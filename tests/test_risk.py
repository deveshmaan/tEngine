...
@pytest.mark.parametrize("stop_pct", [0.0, -0.1, 1.0, 1.5])
def test_position_size_rejects_invalid_stop_loss_pct(stop_pct):
    cfg = RiskLimits(
        daily_pnl_stop=1000,
        per_symbol_loss_stop=1000,
        max_open_lots=2,
    ...
    r = _risk(cfg, capital_base=100000)
    qty = r.position_size(premium=100.0, lot_size=50, stop_loss_pct=stop_pct)
    assert qty == 0
