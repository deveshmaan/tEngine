...
class ExitEngine:
    ...
    async def on_tick(self, instrument_key: str, ltp: float, ts: dt.datetime) -> None:
        ...
        # 2) Target / partial
        elif target1_price and ltp >= target1_price and self.cfg.target1_pct > 0 and not trailing_active:
            partial_qty = self._partial_exit_qty(qty)
            trail_anchor = max(trail_anchor, ltp)
            plan["trailing_active"] = True
            plan["trail_anchor_price"] = trail_anchor
            plan["trailing_stop"] = self._compute_trailing_stop(entry_price, trail_anchor, stop_price, trailing_stop)
            if partial_qty > 0:
                reason = "TARGET1"
                exit_qty = partial_qty
                plan["partial_filled_qty"] = partial_qty
            else:
        ...
        # 4) Time stop
        if not reason and self.cfg.max_holding_minutes > 0:
            age_minutes = (ts_dt - entry_ts).total_seconds() / 60.0
            if age_minutes >= self.cfg.max_holding_minutes:
    ...
    def _exits_enabled(self) -> bool:
        return any(
            [
                ...
                self.cfg.max_holding_minutes > 0,
            ]
        )
